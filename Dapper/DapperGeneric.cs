...
namespace Wpfmain . Dapper
{
    ...
    [Serializable]
    public class DapperGeneric//<T> where T : GenericClass
    {
        ...
        public static dynamic ExecuteWithDapper ( string SqlCommand, List<string [ ]> ArgsList, out int result, out string error )
        {
            ...
            using ( IDbConnection db = new SqlConnection ( ConString ) )
            {
                try
                {
                    ...
                    for ( int y = 0 ; y < ArgsList . Count ; y++ )
                    {
                        Arguments = ArgsList [ y ];
                        // iterate thru each set of arguments we may have received
                        if ( Arguments [ y ] == null || Arguments [ y ] == "" )
                            break;
                        // get the var name and data field name first
                        if ( Arguments [ y ] != null && Arguments [ y ] != "" )
                        {
                            // Sanity chack first
                            if ( Arguments [ y ] . Contains ( "@" ) == false || Arguments [ y + 1 ] == "" )
                            {
                                if ( Arguments [ y ] . Contains ( "@" ) == false || Arguments [ y + 1 ] == "" )
                                {
                                    error = $"The data revied does not include a required variable name or data type";
                                    continue;       // No argument name or data value, so ignore it
                                }
                            }
                            // carry on, we do have some arguments
                            // check for data type
                            if ( Arguments [ y + 2 ] != null && Arguments [ y + 2 ] != "" )
                            {
                                // OK, have we got  a  direction ?
                                if ( Arguments [ y + 3 ] != null && Arguments [ y + 3 ] != "" && Arguments [ y + 3 ] != "" )
                                    // yes, use it, but check for direction first
                                    if ( Arguments [+y+ 3 ] == "INPUT" )
                                        Params . Add ( Arguments [ 0 ], Arguments [ 1 ], GetDbType ( Arguments [ y +2 ] ), ParameterDirection . Input, arg1 . Length );
                                    else if ( Arguments [y + 3 ] == "OUTPUT" )
                                        Params . Add ( Arguments [ 0 ], Arguments [ 1 ], GetDbType ( Arguments [ y+ 2 ] ), ParameterDirection . Output, arg1 . Length );
                                    else
                                    {       // defaullt to STRING
                                        Params . Add ( Arguments [ 0 ], Arguments [ 1 ], GetDbType ( Arguments [ y + 2 ]), ParameterDirection . Input, arg1 . Length );
                                    }
...
/*
                                ...
                                else if ( typeof ( T ) == typeof ( GenericClass ) )
                                                                {
